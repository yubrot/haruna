/**
 * Reusable Terminal UI component for the haruna Web UI.
 *
 * Provides a full session panel with header (mode/theme/windowed controls),
 * content area (interactive or raw view), and input area. Connects to a
 * WebSocket server at the given URL to receive frames and send input.
 *
 * @module
 */
...
/**
 * Full session panel UI.
 *
 * Connects to the WebSocket server at `wsUrl`, displays scene events
 * in interactive or raw mode, and provides an input area for sending
 * text back to the session.
 *
 * @param props.wsUrl - WebSocket URL to connect to
 */
export function Terminal({ wsUrl }: { wsUrl: string }): JSX.Element {
  ...
  const { connected, send } = useChannel(wsUrl, onReceive);
...
/**
 * Manages theme via the `data-theme` attribute on `<html>`.
 *
 * All components sharing this hook stay in sync because a
 * MutationObserver watches for external attribute changes.
 */
export function useDocumentTheme(): [Theme, () => void] {
  const read = (): Theme =>
